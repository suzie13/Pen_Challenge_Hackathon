...
def move(Centroid):
    ...
    Px=float(Ocx - Pcx+Error)
    Py=float(Ocy - Pcz+Error)
    Pz=float(Ocz - Pcy+Error)

    print(f"px={Px}")
    print(f"py={Py}")
    print(f"pz={Pz}")
    #if P
    #robot.gripper.release( delay= 1.0)
    ...
    waist = float(np.arctan(Py/Px))+ Error_waist
    print(f"waist={waist}")
    if waist!=0:
        robot.arm.set_single_joint_position(joint_name='waist',position=waist)
    
        #robot.arm.set_single_joint_position(joint_name='shoulder',position=shoulder)
        #robot.arm.set_single_joint_position(joint_name='elbow',position=elbow)
        #robot.gripper.grasp( delay= 4.0)
    ...
    robot.arm.set_ee_cartesian_trajectory(x=Px-p[0],z=Pz-p[2])
    xe = Px-p[0]
    ye = Py-p[1]
    ze = Pz-p[2]
    print(f"xe={xe}")
    ...
    for i in range(2):
        if p[i]-Centroid[i]:
            robot.arm.set_ee_cartesian_trajectory(x=Px, z=Pz)
'''
        
    #robot.arm.set_ee_cartesian_trajectory(x=2, z=3)
    #robot.gripper.grasp( delay= 1.0)
...
def move3(Centroid):
    ...
    if waist!=0:
        #robot.arm.set_single_joint_position(joint_name='waist',position=waist)
        #robot.arm.set_single_joint_position(joint_name='shoulder',position=shoulder)
        robot.arm.set_single_joint_position(joint_name='elbow',position=wrist_angle)
        #robot.gripper.grasp( delay= 4.0)
        '''
# Create a pipeline
...
try:
    while True:
        ...
        if p_counter > 100:
            Xe, Ye, Ze = move(centroid)
            if (Xe < abs(0.01) and Ye< abs(0.01) and Ze < abs(0.01)):
                robot.gripper.grasp( delay= 5.0)
